...
def login():
    if request.method == 'POST':
        ...
        password = request.form.get('password1') #getting the password from the form

        user = User.query.filter_by(email=email).first()  #querying the database for the user with the email
        if user:
